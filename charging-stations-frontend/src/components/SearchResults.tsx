  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-6 mb-8">
      <h3 className="text-lg font-semibold text-gray-800 mb-4">
        Charging Stations in Postal Code {activePostalCode}
      </h3>
      
      {isLoadingPostalCode && (
        <div className="flex items-center space-x-2 py-4">
          <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-blue-600"></div>
          <span className="text-gray-600">Loading stations for postal code...</span>
        </div>
      )}
      ...
      {!isLoadingPostalCode && !postalCodeError && postalCodeStations.length > 0 && (
        <div className="space-y-3">
          <div className="text-sm text-gray-600 mb-3">
            Found {postalCodeStations.length} charging station{postalCodeStations.length !== 1 ? 's' : ''}
          </div>
